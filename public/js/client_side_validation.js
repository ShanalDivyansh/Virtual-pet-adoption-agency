...
if (questionnaire_form) {
  questionnaire_form.addEventListener("submit", (event) => {
    ...
    let errorMessages = [];
    let selectedPetType = document.querySelector(
      'input[name="petType"]:checked'
    );
    let checkSelectedPetAgeGroup = document.querySelectorAll(
      'input[name="petAgeGroup"]:checked'
    );
    let selectedPetGender = document.querySelector(
      'input[name="petGender"]:checked'
    );
    let checkSelectedPetSize = document.querySelectorAll(
      'input[name="petSize"]:checked'
    );
    let houseTrainPreference = document.querySelector(
      'input[name="houseTrained"]:checked'
    );
    let checkSelectedEnergyLevel = document.querySelectorAll(
      'input[name="energyLevel"]:checked'
    );
    let specialNeedPreference = document.querySelector(
    ...
    if (
      ...
      // answer 2
      let selectedPetAgeGroup = Array.from(checkSelectedPetAgeGroup)
        .map((checkbox) => checkbox.value.trim().toLowerCase())
        .join(",");
      // answer 3
      selectedPetGender = selectedPetGender.value.trim().toLowerCase();
      // answer 4
      let selectedPetSize = Array.from(checkSelectedPetSize)
        .map((checkbox) => checkbox.value.trim().toLowerCase())
        .join(",");
      // answer 5
      houseTrainPreference = houseTrainPreference.value.trim().toLowerCase();
      // answer 6
      let selectedEnergyLevel = Array.from(checkSelectedEnergyLevel)
        .map((checkbox) => checkbox.value.trim().toLowerCase())
        .join(",");
      // answer 7
      ...
      if (
        !selectedPetAgeGroup.includes("puppy") &&
      ...
      if (
        !selectedPetSize.includes("small") &&
      ...
      if (
        !selectedEnergyLevel.includes("low") &&
