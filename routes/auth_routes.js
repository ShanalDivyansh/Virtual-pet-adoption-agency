  ...
  .post(async (req, res) => {
    ...
    if (petType === undefined ||
      ...
      if (Array.isArray(petAgeGroup))
        ...
        petAgeGroup = [petAgeGroup.trim().toLowerCase()];
      // answer 3
      ...
      if (Array.isArray(petSize))
        ...
        petSize = [petSize.trim().toLowerCase()];
      // answer 5
      ...
      if (Array.isArray(energyLevel))
        ...
        energyLevel = [energyLevel.trim().toLowerCase()];
      // answer 7  
      ...
      if (!array1ContainsAllElementsOfArray2(petAgeGroup, ['puppy', 'young adult', 'adult', 'senior']))
      ...
      if (!array1ContainsAllElementsOfArray2(energyLevel, ['low', 'medium', 'high', 'very-high']))
    ...
    if (errors.length > 0)
      return res.status(404).render('questionnaire', { title: 'error', errors: errors });
    else {

      try {
        let user = await addUserQuizAns('657c1e411fc6eb6de505c450', petType, petAgeGroup, petGender, petSize, energyLevel, specialNeeds, houseTrained);
      } catch (error) {
        errors.push(error);
      }
    }
